...
class EnhancedLSTM(nn.Module):  # Изменено с GRU на LSTM
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.4):
        super().__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers,  # Заменено на LSTM
    ...
    def forward(self, x):
        output, (h_n, c_n) = self.lstm(x)  # LSTM возвращает (output, (h_n, c_n))
...
def load_trained_model(input_size=3, hidden_size=128):
    model = EnhancedLSTM(input_size=input_size,  # Используем EnhancedLSTM вместо EnhancedGRU
...
def test_intel_stock(window_size=20):
    ...
    # Загрузка модели
    model = load_trained_model(input_size=3)
