...
texts=[]
with open("INTC_data.json","r",encoding='utf-8') as file:
    data = json.load(file)
...
def is_about_stock(text, threshold=0.75):
    ...
    for label, score in zip(result['labels'], result['scores']):
        if label != "Not related to Intel" and score > threshold:
            return True
    return False
...
def get_sentiment(text):
    if len(text)>512:
        text = text[:512]
    return sentiment("This news may affect Intel's stock price. " + text)[0]['label']
...
buff_df.to_csv('Intel_sentiment.csv', index=False)
