...
def is_about_apple(text, threshold=0.75):
    if len(text)>512:
        text = text[:512]
    candidate_labels = ["Apple","iPhone","MacBook","Tim Cook","iOS","App Store","Not related to Apple"]

    result = classifier(text, candidate_labels, multi_label=True)

    # Считаем связанным, если любая тесловская тема выше порога и выше "Not related"
    for label, score in zip(result['labels'], result['scores']):
        if label != "Not related to Apple" and score > threshold:
            return True
    return False
...
def get_sentiment(text):
    if len(text)>512:
        text = text[:512]
    return sentiment("This news may affect Apple's stock price. " + text)[0]['label']

#Функция общего анализа новости
def analyze_news(text):
    if len(text)>512:
        text = text[:512]
    if is_about_apple(text):
...
df_Apple_sentiment = pd.DataFrame(sentiments, columns=['date', 'sentiment'])
df_Apple_sentiment['date'] = pd.to_datetime(df_Apple_sentiment['date'])
df_Apple_sentiment.to_csv('sentiment.csv', index=False)
