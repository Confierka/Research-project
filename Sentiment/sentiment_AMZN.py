...
texts=[]
with open("doneAMZN.json","r",encoding='utf-8') as file:
    data = json.load(file)
...
def is_about_stock(text, threshold=0.6):
    if len(text)>512:
        text = text[:512]
    candidate_labels = [
        "Amazon",
    ...
    for label, score in zip(result['labels'], result['scores']):
        if label != "Not related to Amazon" and score > threshold:
            return True
    return False
...
def get_sentiment(text):
    if len(text)>512:
        text = text[:512]
    return sentiment("This news may affect Amazon's stock price. " + text)[0]['label']
...
buff_df.to_csv('AMZN_sentiment.csv', index=False)
